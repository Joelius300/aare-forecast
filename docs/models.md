...
architecture changes (e.g. new type, new input variable, etc.) and the second is bumped when
...
(takes a while to load). Note that in this first version of Aare Oraku, it's only this one use case and most
things are centered around it, so a lot can and will change when other use cases are introduced.
...
- **Accuracy:** 50% of forecasts have a mean absolute error across all the predicted 36h of **0.4 °C or less**.
  **80% of 0.7 °C or less**. In the worst summer periods, errors are regularly between 0.5 and 1.5 °C at larger lags.
  _Only on measurements, performance will degrade when using forecasts as inputs!_
